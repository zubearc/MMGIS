...
async function onlyExistingFilepaths (paths) {
  ...
  const filesExist = await Promise.all(filePromises).catch((err) => {}) // eslint-disable-line n/handle-callback-err
...
function getTimePath (prepath, suffixPath, time, starttime, returnUrls) {
  ...
  if (dirs) {
    ...
    if (closestTimeWithoutGoingOver) { return `${prepath}${closestTimeWithoutGoingOver}${suffixPath}` }
