  ...
  res.send = function (payload) {
    if (payload.status === 'success') {
      if (payload?.body?.entries && payload.body.entries.length > 0) {
        const config = JSON.parse(payload.body.entries[0].config)
...
function buildBody (webhookBody, injectableVariables) {
  // Fill in the body
  for (const i in webhookBody) {
    const match = INJECT_REGEX.exec(webhookBody[i])
...
function buildUrl (url, injectableVariables) {
  let updatedUrl = url
  let match
  while ((match = INJECT_REGEX.exec(updatedUrl)) !== null) {
    const variable = match[1]
    if (!injectableVariables[variable]) {
      logger(
        'error',
    ...
    let newVariable = injectableVariables[variable]
