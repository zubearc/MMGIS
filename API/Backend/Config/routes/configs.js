...
// const configTemplate = require('../../../templates/config_template')
...
if (
  !Object.hasOwn(process.env, 'HIDE_CONFIG') ||
...
function add (req, res, next, cb) {
  // TODO: bug, where does configTemplate come from?
  // let configTemplate = JSON.parse(JSON.stringify(configTemplate))
  let configTemplate = {}
  ...
  Config.findOne({
    ...
    .then((mission) => {
      if (!mission) {
        ...
        else { res.send({ status: 'failure', message: 'Mission already exists.' }) }
      }
      return null
    })
    .catch((err) => {
    ...
    .then((version) => {
      ...
      Config.create({
        ...
        .catch((err) => {
          ...
          if (cb) { cb({ status: 'failure', message: 'Failed to update mission.' }) } else {
            res.send({
              status: 'failure',
...
router.get('/missions', function (req, res, next) {
  if (req.query.full === 'true') {
    ...
    Config.aggregate('mission', 'DISTINCT', { plain: false })
      .then((missions) => {
        const allMissions = []
        for (let i = 0; i < missions.length; i++) { allMissions.push(missions[i].DISTINCT) }
        allMissions.sort()
...
function relativizePaths (config, mission) {
  const relConfig = JSON.parse(JSON.stringify(config))
  ...
  function setAllKeys (data, prepend) {
    if (typeof data === 'object' && data !== null) {
      for (const k in data) {
        if (typeof data[k] === 'object' && data[k] !== null) { setAllKeys(data[k], prepend) } else if (Array.isArray(data[k])) setAllKeys(data[k], prepend)
        else if (k === 'url' || k === 'demtileurl' || k === 'legend') { if (data[k].indexOf('://') === -1) data[k] = prepend + '' + data[k] }
      }
    } else if (Array.isArray(data)) {
      for (let i = 0; i < data.length; i++) {
        if (typeof data[i] === 'object' && data[i] !== null) { setAllKeys(data[i], prepend) } else if (Array.isArray(data[i])) setAllKeys(data[i], prepend)
      }
    }
  }
      ...
      if (r.status === 'success') {
        ...
        execFile(
          ...
          function (error, stdout, stderr) {
            if (error) return res.send({ status: 'failure', message: '' })
...
function openWebSocket (body, response, info, forceClientUpdate) {
  if (
    !Object.hasOwn(process.env, 'ENABLE_MMGIS_WEBSOCKETS') ||
  ...
  const path = `ws://localhost:${port}${process.env.WEBSOCKET_ROOT_PATH || process.env.ROOT_PATH || ''
    }/`
            ...
            upsert(
              ...
              (resp) => {
                if (resp.status === 'success') {
                  res.send({
                    status: 'success',
                    message: `Successfully removed layer${removedUUIDs.length >= 1 ? 's' : ''
                  ...
                  res.send({
                    status: 'failure',
                    message: `Failed to remove layer${layerUUIDs.length >= 1 ? 's' : ''
                      }: ${resp.message}.`,
