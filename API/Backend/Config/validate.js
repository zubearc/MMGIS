...
const validateStructure = (config) => {
  ...
  if (config.layers == null) { errs.push(err('Configuration is missing the \'layers\' object.', ['layers'])) }
  if (config.tools == null) { errs.push(err('Configuration is missing the \'tools\' object.', ['tools'])) }
...
const isValidLayerName = (name) => {
  const errs = []
  if (name == null) { errs.push(err('Found a layer with name: null.', ['layers[layer].name'])) }
  if (name === '') { errs.push(err("Found a layer with name: ''", ['layers[layer].name'])) }
  if (name === 'undefined') {
    errs.push(
...
const isValidUrl = (layer) => {
  ...
  if (layer.url === '') { errs.push(err(`Layer '${layer.name}' has URL: ''`, ['layers[layer].url'])) }
