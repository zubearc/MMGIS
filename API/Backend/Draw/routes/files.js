/* eslint-disable camelcase, n/no-callback-literal */
...
router.post('/getfiles', function (req, res, next) {
  const Table = req.body.test === 'true' ? UserfilesTEST : Userfiles
...
router.post('/make', function (req, res, next) {
  ...
  Table.create(newUserfile)
    .then((created) => {
      const geojson = req.body.geojson ? JSON.parse(req.body.geojson) : null
      if (
        geojson &&
        geojson.features &&
        ...
        for (let i = 0; i < features.length; i++) {
          ...
          ) { intent = features[i].properties._.intent } else {
        ...
        Userfeatures.bulkCreate(rows, { returning: true })
          .then(function (response) {
            ...
            Filehistories.findAll({
              ...
              .then((historyObj) => {
                const history = historyObj.history.concat(ids)
...
router.post('/change', function (req, res, next) {
  ...
  if (Object.hasOwn(req.body, 'public_editors')) {
    try {
      let publicEditors = null
      if (typeof req.body.public_editors === 'string') {
        publicEditors = req.body.public_editors
          .split(',')
          .map((e) => e.trim())
      }
      toUpdateTo.public_editors = publicEditors
    } catch (err) { }
  }
...
const compile = function (req, res, callback) {
  ...
  const Table = isTest ? UserfilesTEST : Userfiles

  const atThisTime = req.query.time || Math.floor(Date.now())
  ...
  Table.findAll({
    ...
    for (let f = 0; f < files.length; f++) {
      sequelize
        ...
        .then(([results]) => {
          const bestHistory = results.length > 0 ? results[0].history : []
    ...
    function processFeatures (features) {
      sequelize
        ...
        .then(([results]) => {
          const hierarchy = []
          const intentOrder = ['roi', 'campaign', 'campsite', 'signpost']
          const excludeIntents = ['polygon', 'line', 'point', 'text', 'arrow']
          const flatHierarchy = []
          const issues = []
          // const changes = []
          ...
          for (let f = 0; f < features.length; f++) {
            ...
            if (!excludeIntents.includes(features[f].intent)) {
              for (let r = 0; r < results.length; r++) {
                if (results[r].id === features[f].id) {
                  const childProps = JSON.parse(results[r].associated_properties)
            ...
            const featureProps = JSON.parse(features[f].properties)
            flatHierarchy.push({
              feature: features[f],
              id: features[f].id,
          ...
          function addParents () {
            for (let i = 0; i < flatHierarchy.length; i++) {
              ...
              if (
                ...
                const intentOrderReversed = JSON.parse(
                  JSON.stringify(intentOrder)
                )
                intentOrderReversed.reverse()
                const intentId = intentOrderReversed.indexOf(
          ...
          function depthTraversal (node, depth) {
            for (let i = 0; i < node.length; i++) {
              ...
              if (node[i].children.length > 0) { depthTraversal(node[i].children, depth + 1) }
            }
          }
          ...
          function addFeatureData (node, depth) {
            for (let i = 0; i < features.length; i++) {
              const f = features[i]
              if (node.id === f.id) {
                const properties = JSON.parse(f.properties)
                const feature = {}
                properties._ = {
                  id: f.id,
                  file_id: f.file_id,
          ...
          function addOutcasts () {
            ...
            for (let i = 0; i < allIds.length; i++) {
              if (includedIds.indexOf(allIds[i]) === -1) { outcastIds.push(allIds[i]) }
            }
            ...
            for (let i = 0; i < flatHierarchy.length; i++) {
              if (outcastIds.indexOf(flatHierarchy[i].id) !== -1) {
                if (
                  ...
                  const outcast = JSON.parse(JSON.stringify(flatHierarchy[i]))
          ...
          function findIssues () {
            ...
            for (let i = 0; i < flatHierarchy.length; i++) {
              const node = flatHierarchy[i]
              const intent = node.intent
              const props = JSON.parse(node.feature.properties)
              ...
              if (props.uuid == null) {
                ...
                const uuidKeys = Object.keys(uuidsFound)
                const uuidI = uuidKeys.indexOf(props.uuid)
                if (uuidI >= 0) {
                  issues.push({
                    severity: 'error',
                    antecedent: {
              ...
              if (props.name == null) {
                ...
                const nameKeys = Object.keys(namesFound)
                const nameI = nameKeys.indexOf(props.name)
                if (nameI >= 0) {
                  issues.push({
                    severity: 'error',
                    antecedent: {
              ...
              if (intentOrder.indexOf(intent) === -1) continue
              const parentIntent = intentOrder[intentOrder.indexOf(intent) - 1]
              ...
              const ints = node.possibleChildren.directIntersects
            ...
            sequelize
              ...
              .then(([published_family_tree]) => {
                if (
                  ...
                  const tree = JSON.parse(published_family_tree[0].value)
                  const fh = tree.flatHierarchy
                  const oldFeatures = {}
                  const newFeatures = {}
                  const added = []
                  const changed = []
                  ...
                  const newFeatureUUIDs = Object.keys(newFeatures)
                  const oldFeatureUUIDs = Object.keys(oldFeatures)
...
router.post('/publish', function (req, res, next) {
  const Table = req.body.test === 'true' ? UserfilesTEST : Userfiles
  const Histories = req.body.test === 'true' ? FilehistoriesTEST : Filehistories

  const time = Math.floor(Date.now())

  // Check that user belongs to sel group
  if (req.groups[req.leadGroupName] != true) { // eslint-disable-line eqeqeq
  ...
  function publishToPublished (cb) {
    const Publisheds = req.body.test === 'true' ? PublishedTEST : Published
            ...
            Publisheds.destroy({
              ...
              for (let i = 0; i < fH.length; i++) {
                const feature = {
                  id: fH[i].id,
                  intent: fH[i].intent,
                  parent: Object.hasOwn(fH[i].parent, 'id')
                    ? fH[i].parent.id
                    : null,
...
router.post('/gethistory', function (req, res, next) {
  const Table = req.body.test === 'true' ? FilehistoriesTEST : Filehistories
