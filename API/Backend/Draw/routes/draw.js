...
const clipUnder = function (
  ...
  const Features = req.body.test === 'true' ? UserfeaturesTEST : Userfeatures
  const Histories = req.body.test === 'true' ? FilehistoriesTEST : Filehistories
      ...
      sequelize
        ...
        .then(([results]) => {
          ...
          function addLoop (i) {
            ...
            const clippedFeature = Object.assign({}, newFeature)
  ...
  return new Promise((resolve, reject) => {
    ...
    function _getNextIncrement (value, t, layer, existingProperties) {
      ...
      if ((response.newValue || '').indexOf('#') !== -1) {
        ...
        if (numVal !== '#') {
          ...
          if (existingProperties[t.field] === response.newValue) {
            ...
            if (count > 1) { response.error = `Incrementing field: '${t.field}' is not unique` }
          } else {
            // In case a manual change, make sure the id is unique
            if (usedValues.indexOf(numVal) !== -1) { response.error = `Incrementing field: '${t.field}' is not unique` }
          }
        }
      }
      ...
      for (let i = 0; i < layer.length; i++) {
        ...
        const geojson = layer[i]
        if (geojson?.properties?.[t.field] != null) {
          const featuresVal = geojson?.properties?.[t.field]
          if (
            (value || '').indexOf('#') === -1 &&
...
const add = async function (
  ...
  if (failedTemplate !== false) {
    if (typeof failureCallback2 === 'function') { failureCallback2(failedTemplate) }
    return
  }

  const Files = req.body.test === 'true' ? UserfilesTEST : Userfiles
  const Features = req.body.test === 'true' ? UserfeaturesTEST : Userfeatures
  const Histories = req.body.test === 'true' ? FilehistoriesTEST : Filehistories
    ...
    if (!file) {
      ...
      Features.findAll({
        ...
        .then((level) => {
          ...
          const geom = JSON.parse(req.body.geometry)
          ...
          if (req.body.clip === 'under') {
            clipUnder(
              ...
              (createdId, createdIntent) => {
                if (typeof successCallback === 'function') { successCallback(createdId, createdIntent) }
              },
              (err) => {
                if (typeof failureCallback2 === 'function') { failureCallback2(err) }
              }
            )
          } else {
                ...
                if (req.body.to_history) {
                  ...
                  if (req.body.clip === 'over') {
                    clipOver(
                      ...
                      time,
                      () => {
                        if (typeof successCallback === 'function') { successCallback(created.id, created.intent) }
                      },
                      (err) => {
                        if (typeof failureCallback2 === 'function') { failureCallback2(err) }
                      }
        ...
        Features.findOne({
          ...
          .then((feature) => {
            if (!feature && !req.body.addIfNotFound) {
              ...
              const newAttributes = feature.dataValues

              delete newAttributes.id
              delete newAttributes.properties._
              newAttributes.extant_start = time

              if (Object.hasOwn(req.body, 'parent')) { newAttributes.parent = req.body.parent }
              if (Object.hasOwn(req.body, 'keywords')) { newAttributes.keywords = req.body.keywords }
              if (Object.hasOwn(req.body, 'intent')) { newAttributes.intent = req.body.intent }
              if (Object.hasOwn(req.body, 'properties')) { newAttributes.properties = req.body.properties }
              if (Object.hasOwn(req.body, 'geometry')) {
              ...
              if (
                Object.hasOwn(req.body, 'reassignUUID') &&
              ...
              Features.create(newAttributes)
                .then((created) => {
                  ...
                  const createdUUID = JSON.parse(created.properties).uuid
                  const createdIntent = created.intent
            ...
            [Sequelize.Op.or]: {
              [Sequelize.Op.and]: {
                ...
                [Sequelize.Op.or]: {
                  ...
                  // TODO: why twice ?
                  extant_end: null // eslint-disable-line no-dupe-keys
                }
              },
              trimmed_at_final: {
...
router.post('/merge', function (req, res, next) {
  ...
  if (req.body.ids.indexOf(req.body.prop_id) === -1) { req.body.ids.push(req.body.prop_id) }
    ...
    if (!file) {
      ...
      Features.findOne({
        ...
        sequelize
          ...
          .then(([results]) => {
            const oldIds = req.body.ids.map(function (id) {
            ...
            function addLoop (i) {
              ...
              const mergedFeature = JSON.parse(JSON.stringify(feature))
...
router.post('/split', function (req, res, next) {
  ...
  const splitFeature = JSON.parse(req.body.split)
  ...
  Files.findOne({
    ...
    .then((file) => {
      if (!file) {
        ...
        const geom = splitFeature.geometry
        geom.crs = { type: 'name', properties: { name: 'EPSG:4326' } }
        ...
        sequelize
          ...
          .then(([results]) => {
            ...
            for (let i = 0; i < results.length; i++) {
              for (let j = 0; j < results[i].st_split.geometries.length; j++) {
...
const makeMasterFilesTEST = (leadGroupName, callback) => {
  const intents = ['roi', 'campaign', 'campsite', 'trail', 'signpost']
