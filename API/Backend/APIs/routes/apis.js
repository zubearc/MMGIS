...
router.get('/layer=:layer_name', function (req, res, next) {
  const layerName = req.params.layer_name
  const query1 =
  ...
  db.any(query1)
    .then(function (d1) {
      // Check the result
      const queryTOexecute = d1[0].sqlstmt
      const query2 =
...
router.get('/all_targets_elev/', function (req, res, next) {
  ...
  db.any(query)
    .then(function (result) {
      ...
      const numOfBins = 80
      const minElev = parseFloat(data[0])
      const maxElev = parseFloat(data[data.length - 1])
      const diffBetweenMinMax = Math.abs(maxElev - minElev)

      const binSize = diffBetweenMinMax / numOfBins
...
router.get('/all_layers/', function (req, res, next) {
  const query = 'SELECT layer_name, layer_type, table_name_refered FROM layers;'
...
router.get(
  ...
  function (req, res, next) {
    const query = 'SELECT target_plan FROM Targets;'
    // Get the data from database
    db.any(query)
      .then(function (data) {
        res.send(data)
...
router.get('/instList', function (req, res, next) {
  const query = 'SELECT inst_type FROM Instruments;'
  // Get the data from database
  db.any(query)
    .then(function (data) {
      res.send(data)
...
router.get('/inst=:instrument/data_min_avg_max&mat=:material', function (
  ...
  const query = 'SELECT ' + material + ' FROM data_' + instrument + ' ;'
  ...
  db.any(query)
    .then(function (d) {
      ...
      if (l % 2 === 1) {
        ...
        const a = ((data[l / 2 - 1] + data[l / 2]) / 2) * 1.0
        median =
      ...
      // Set the bin size
      const binSize = Math.min(data.length, 40)

      const binGap = (max - min) / binSize
      ...
      for (const i in data) {
        const g = parseInt((data[i] - min) / binGap, 10)
        bins[g] += 1
...
router.get('/final_query/data=:final_data', function (req, res, next) {
  ...
  // the interface.
  // const data = req.params.final_data
...
router.get(
  ...
  function (req, res, next) {
    ...
    const getQueryRecord = 'SELECT * FROM user_queries WHERE id=$1;'
...
router.post(
  ...
  function (req, res, next) {
    ...
    const loadedQuery = req.body.query
...
router.get(
  ...
  function (req, res, next) {
    const queryID = req.params.query_id
    const deleteQuery = 'DELETE FROM user_queries WHERE id=$1;'
...
async function runFinalQuery (req, res, next) {
  ...
  const queryDescription = data.description
  ...
  for (i = 0; i < components.length; i++) {
    ...
    if (compType === 'N' || compType === 'S') {
      ...
      if (compType === 'N') {
        ...
        const inputRange = components[i].input
      ...
      if (compType === 'S') {
        // Get the input value
        const valueForField = components[i].input
        const isnum = /^\d+$/.test(valueForField)
    ...
    if (compType === 'G') {
      ...
      const shape = components[i].geomOp
      const coordinates = components[i].coordinates

      if (shape === 'elevation') {
        const elevation = components[i].input
        geometryPartQuery += ST_Z(elevation)
      }

      if (shape === 'line_buffer' || shape === 'circle_buffer') {
        // Create line buffer or circle buffer
        const distance = components[i].input
      ...
      if (shape === 'intersection') {
        const layerData = components[i].input
        const intersectQuery = await ST_Intersect(coordinates, layerData)
...
function ST_Intersect (coordinates, selectedLayerData) {
  return new Promise(resolve => {
    const layerName = selectedLayerData[0]
    const layerType = selectedLayerData[1]
    let geomAsText = ''
    let coords = ''
    let i
    for (i = 0; i < coordinates.length - 1; i++) {
    ...
    db.any(q)
      .then(function (geoms) {
        ...
        if (layerType === 'Points') {
          ...
          for (let i = 0; i < geoms.length - 1; i++) {
            const GeojsonObj = JSON.parse(geoms[i].st_asgeojson)
            const lng = GeojsonObj.coordinates[0]
            const lat = GeojsonObj.coordinates[1]
          ...
          // Set up last round
          const lastGeojsonObj = JSON.parse(geoms[geoms.length - 1].st_asgeojson)
          const lastLng = lastGeojsonObj.coordinates[0]
          const lastLat = lastGeojsonObj.coordinates[1]
        ...
        if (layerType === 'LineString') {
          ...
          for (let i = 0; i < geoms.length - 1; i++) {
            const GeojsonObj = JSON.parse(geoms[i].st_asgeojson)
            MultiLineStr += '('
            const coordins = GeojsonObj.coordinates[0]
            for (let j = 0; j < coordins.length - 1; j++) {
              const lng = coordins[j][0]
              const lat = coordins[j][1]
          ...
          MultiLineStr += '('
          const lastCoordins = JSON.parse(geoms[geoms.length - 1].st_asgeojson)
            .coordinates[0]
          for (let k = 0; k < lastCoordins.length - 1; k++) {
            const lng = lastCoordins[k][0]
            const lat = lastCoordins[k][1]
            MultiLineStr += lng + ' ' + lat + ','
          }
        ...
        if (layerType === 'Polygon') {
          ...
          const geoJSONobj = JSON.parse(geoms[0].st_asgeojson)
          const polygonCoords = geoJSONobj.coordinates[0]
          let counter = 0

          let PolygonStr = " ST_GeomFromText('POLYGON(("

          for (const t in polygonCoords) {
      ...
      .catch(function (err) {
        console.error(err)
        // TODO: what is this next(err) for? It's not defined here
        // return next(err)
      })
  }, 2000)
}
