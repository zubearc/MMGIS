    ...
    .then((result) => {
      if (result) {
        ...
        if (type === 'geojson') {
          ...
          const oLat = Math.abs(ne.lat - sw.lat) / (4096 / 256)
          const oLng = Math.abs(ne.lng - sw.lng) / (4096 / 256)
          const ne2 = { lat: ne.lat + oLat, lng: ne.lng + oLng }
          const sw2 = { lat: sw.lat - oLat, lng: sw.lng - oLng }
